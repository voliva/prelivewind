...
const calendarSvg = (className, onClick, direction?:'right'|'left') =>
    <svg width="22" height="23" className={className} onClick={onClick} style="fill:inherit; stroke:inherit">
...
const Icon = ({type, className, onClick, fill, style}: IconProps) => {
    ...
    switch(type) {
        case 'arrowBack':
            return <svg style={style} width='25' height='20' className={className} onClick={onClick}>
                <path d='m 2,10 c 2.5,2.5 4.5,5.5 7,8 M 23,10 c -7,0 -14,0 -21,0 C 4.5,7.5 6.5,4.5 9,2' />
            </svg>
        case 'star':
            return <svg style={style} width='25' height='23' className={className} onClick={onClick}>
        ...
        case 'windArrow':
            return <svg style={style} viewBox="0 0 8.173 11.23" className={className} onClick={onClick}>
                <path fill="#afafaf" stroke="#000" stroke-width=".5" d="M.25.25l4.096 10.73L7.923.31l-3.75 3.778z" stroke-linejoin="round"/>
            </svg>
        case 'search':
            return <svg style={style} className={className} onClick={onClick} width="5" height="5">
                <circle cx="2.25" cy="2.27" r="1.93" fill="none" stroke="#000" stroke-width=".65" stroke-linejoin="round"/>
                <path fill="none" stroke="#000" stroke-width=".76" d="M3.85 3.77L5.7 5.62" stroke-linecap="round"/>
            </svg>
        default:
